...
code_freq_new_s <- codes_vars_s2 %>%
  mutate(br_pos = as.numeric(if_any(37:54, ~. == 1)), 
         ba_pos = as.numeric(if_any(21:36, ~. == 1)),
         br_reg = as.numeric(if_any(c(37:44, 50), ~. ==1)),
         br_soc = as.numeric(if_any(c(45:49, 51), ~. == 1)), 
         br_dat = as.numeric(if_any(c(52), ~. == 1)), 
         br_eco = as.numeric(if_any(c(53, 54), ~. == 1)), 
         ba_reg = as.numeric(if_any(c(21, 22), ~. == 1)),
         ba_dat = as.numeric(if_any(c(23:25), ~. == 1)), 
         ba_eco = as.numeric(if_any(c(26), ~. == 1)), 
         ba_soc = as.numeric(if_any(c(27:36), ~. == 1)))

brba_sum_s <- code_freq_new_s %>%
  summarise(n_br = sum(br_pos), freq_br = n_br/21, #total number of surveys not including EcoCast
  ...
  mutate(percent_label = scales::percent(value, accuracy = 1), 
         ...
         code_level = if_else(br_ba %in% c("freq_ba", "freq_br"), "Overall bridge or barrier", 
                              if_else(br_ba %in% c("freq_ba_e", "freq_br_e"), "Ecological bridge or barrier", 
                              if_else(br_ba %in% c("freq_ba_d", "freq_br_d"), "Data bridge or barrier",
                              if_else(br_ba %in% c("freq_ba_r", "freq_br_r"), "Implementation bridge or barrier",
                              if_else(br_ba %in% c("freq_ba_s", "freq_br_s"), "Social bridge or barrier", NA))))),
...
brba_sum_s$code_level <- factor(brba_sum_s$code_level, levels = c("Ecological bridge or barrier", "Data bridge or barrier", "Implementation bridge or barrier","Social bridge or barrier", "Overall bridge or barrier"))
...
code_sum_s <- code_freq_s %>%
  ...
  mutate(percent_label = scales::percent(value, accuracy = 1), 
         ...
         code_level = if_else(br_ba %in% c("Ecological barrier", "Ecological bridge"), "Ecological bridge or barrier", 
                              if_else(br_ba %in% c("Data barrier", "Data bridge"), "Data bridge or barrier",
                              if_else(br_ba %in% c("Regulation barrier", "Regulation bridge"), "Implementation bridge or barrier",
                              if_else(br_ba %in% c("Social barrier", "Social bridge"), "Social bridge or barrier", NA)))), 
         code_brba = if_else(br_ba %in% c("Data bridge", "Ecological bridge", "Social bridge", "Regulation bridge"), "bridge", "barrier")) %>%
  group_by(code_level) %>%
  distinct()

code_sum_s$code_level <- factor(code_sum_s$code_level, levels = c("Ecological bridge or barrier", "Data bridge or barrier","Implementation bridge or barrier", "Social bridge or barrier"))
