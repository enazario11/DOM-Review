...
code_rel2 <- read.csv(here("data/br_ba_code_lit.csv"))

#survey data
code_vars_s <- read.csv(here("data/surveys/code_w_vars.csv"))
code_freq_s <- read.csv(here("data/surveys/code_freq.csv"))
code_rel_s <- read.csv(here("data/surveys/br_ba_code_survey.csv"))
...
### Result 2.2: Heat maps of theme co-occurence
...
ggplot(code_rel_melt_h, aes(x = code2, y = code1)) +
...
theme_minimal() + 
theme(axis.text.x = element_text(angle = 90, hjust = 0, vjust = 0.3, size = 13), 
      axis.text.y = element_text(size = 13))+
geom_text(aes(label = total))+
scale_color_manual(values = c("black", "#00000000"))+
guides(color = "none")

#ggsave("figures/ms_figs/heat_tile.png", width = 11, height = 12, units = c("in"))
...
code_rel_melt2_s$code2[code_rel_melt2_s$code2 == "Interest holder conflicts"] <- "Interest-holder conflicts"
code_rel_melt2_s$code2[code_rel_melt2_s$code2 == "Missing cross program comparisons"] <- "Missing cross-program comparisons"
code_rel_melt2_s$code2[code_rel_melt2_s$code2 == "Lack of near real time data"] <- "Lack of near real-time data"

code_rel_melt_h_s <- code_rel_melt2_s %>%
  #filter(total >= 11) %>%
  mutate(#bor_box = ifelse(total >= 11, "TRUE", "FALSE"), 
        #bor_box = as.factor(bor_box), 
        code1 = as.factor(code1), 
...
#relevel
code_rel_melt_h_s$code1 <- factor(code_rel_melt_h_s$code1, levels = c(as.character(unique(code_rel_melt_h_s$code1))))

code_rel_melt_h_s$code2 <- factor(code_rel_melt_h_s$code2, levels =
c("Unpredictable bycatch patterns", "Missing cross-program comparisons",	"Gaps and inaccuracy",	"Lack of near real-time data", "Cost",	"Discontinuity",	"Confidentiality",	"Lack of incentives",	"Volunteer programs reduce participation",	"Regulated programs reduce participation",	"External factors reducing participation",	"Poor communication and outreach",	"Interest-holder conflicts",	"Stigmatization", "Difficulty assessing impact",	"Limited relevance"))

#plot
ggplot(code_rel_melt_h_s, aes(x = code2, y = code1)) +
geom_tile(aes(fill = total)) + 
scale_fill_gradient2(high = "red")+
ylab("")+
xlab("")+
...
#ggsave("figures/ms_figs/heat_tile_survey.png", width = 8.5, height = 8, units = c("in"))

```

![](images/heat_tile_ai.png){fig-align="center" width="10in", height = "12in"}

![](images/heat_tile_survey_ai.png){fig-align="center" width="10in", height = "12in"}
...
freq_job_survey$bridge_barr <- factor(freq_job_survey$bridge_barr, levels = c("Barrier", "Bridge"))
...
#stacked bar plots
freq_job_survey2$code_cat <- factor(freq_job_survey2$code_cat, levels = c("Data bridge", "Ecological bridge", "Social bridge", "Implementation bridge", "Data barrier", "Ecological barrier", "Social barrier", "Implementation barrier"))
#freq_job_survey2$bridge_barr <- factor(freq_job_survey2$bridge_barr, levels = c())
  
ggplot(freq_job_survey2, aes(fill=code_cat, y=value, x=fish_ship)) + 
    geom_bar(position="stack", stat="identity") +
    scale_fill_manual(values = c("#CFE2F3", "#6D8C9E","#516875","#28343B", "#F1DA9F", "#FED240", "#DBA400", "#614900")) +
    ...
    xlab("") + 
    ylab("Percent of theme (%)")

#ggsave("figures/ms_figs/industry/stack_survey.png", width = 8, height = 6, units = c("in"))
...
ggradar::ggradar(top_bridge,
        values.radar = c(0, 11, 22), 
        background.circle.colour = "white",
        axis.line.colour = "gray60",
        gridline.min.colour = "gray60",
        gridline.mid.colour = "gray60",
        gridline.max.colour = "gray60",
        group.colours = c("#A4BFB8", "#3B5948"),
...
ggradar::ggradar(top_barrier,
        ...
        gridline.mid.colour = "gray60",
        gridline.max.colour = "gray60",
        group.colours = c("#A4BFB8", "#3B5948"),#007172
        legend.position = "none", 
        fill = TRUE, 
        axis.label.size = 3)
...
freq_job_lit <- code_vars %>% 
  ...
  summarise(total = sum(total), 
            .groups = "drop") %>%
  filter(fish_ship == "Bycatch" | fish_ship == "Ship strikes")
...
freq_job_lit$bridge_barr <- factor(freq_job_lit$bridge_barr, levels = c("Barrier", "Bridge"))
...
#stacked bar plots
freq_job_lit2$code_cat <- factor(freq_job_lit2$code_cat, levels = c("Data bridge", "Ecological bridge", "Social bridge", "Data barrier", "Ecological barrier", "Social barrier"))
  
ggplot(freq_job_lit2, aes(fill=code_cat, y=value, x=fish_ship)) + 
    geom_bar(position="stack", stat="identity") +
    scale_fill_manual(values = c("#CFE2F3", "#6D8C9E","#516875", "#F1DA9F", "#FED240", "#DBA400")) +
    facet_wrap(~bridge_barr) +
    labs(fill = "") +
    guides(fill=guide_legend(nrow=2,byrow=TRUE)) +
    theme_ms() +
...
#stacked bar plots
freq_pos_survey2$code_cat <- factor(freq_pos_survey2$code_cat, levels = c("Data bridge", "Ecological bridge", "Social bridge", "Implementation bridge", "Data barrier", "Ecological barrier", "Social barrier", "Implementation barrier"))

ggplot(freq_pos_survey2, aes(fill=code_cat, y=value, x=position)) + 
    geom_bar(position="stack", stat="identity") +
    scale_fill_manual(values = c("#CFE2F3", "#6D8C9E","#516875","#28343B", "#F1DA9F", "#FED240", "#DBA400", "#614900")) +
    facet_wrap(~bridge_barr, ncol = 1) +
    labs(fill = "") +
    theme_ms() +
    theme(legend.position = "top", 
...
ggsave("figures/ms_figs/supp/position/stack_survey.png", width = 8, height = 11, units = c("in"))
